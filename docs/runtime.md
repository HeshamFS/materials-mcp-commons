...
Engine distribution `0.1.0a8` implements the plugin-agnostic runtime and Scientist Kit boundary: exact profile loading, contained manifest-package validation, registration, bounded discovery, inspection, activation leases, exact-owner dispatch, deterministic effect planning and authorization, canonical context measurement and projection, durable local run/artifact state, deterministic empty-workspace scaffolding and package construction, structural conformance reports, explicit migration assessment, manifest-derived capability references, an optional official-SDK MCP adapter, metadata-only operational signals, and integrity-checked state snapshots.
