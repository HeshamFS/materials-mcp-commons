...
Permissions reject wildcard names and scopes. R1 requires exact permissions and any declared quota but no approval. R2 and R3 require an unexpired standard-or-strong approval. R4 requires strong confirmation and recovery. An approval can authorize only one grant; a grant can be consumed only once. Consumption reserves all quota charges atomically and returns an immutable receipt. Actual dispatch atomically redeems that receipt once immediately before handler invocation, and approval expiry is rechecked against a trusted policy-engine clock. A failed replay, binding check, approval check, permission check, quota reservation, or receipt redemption fails closed.

The dispatcher accepts effectful work only when its `PolicyEngine` atomically redeems that exact durable receipt immediately before handler invocation. The older non-redeeming `verify_receipt` method remains available for compatibility: it never changes receipt lifecycle state or authorizes execution by itself, but failed verification still appends a bounded metadata-only denial audit event. R2-R4 run creation applies the same strengthened non-redeeming durable binding check before dispatch. Handler rejection, failure, or cancellation after redemption does not refund quota or make the receipt reusable. Version-1 policy stores migrate transactionally to version 2; previously consumed receipts are marked `legacy-closed` because their prior dispatch state is unknowable, so their reserved quota and approvals remain consumed rather than risking replay. Version-1 policy state can be snapshotted before migration; version-1 recovery snapshots remain inspectable and restorable, then migrate on first open.
