...
All host and dispatcher failures use the exact profile structured-error envelope with cause, stage, evidence, retryability, corrective next action, and occurrence time. Execute success may carry any JSON root beneath `result`; the host preserves arrays, scalars, objects, and null without changing their JSON types. Values such as `Decimal` that the SDK would serialize with a different wire type fail with `RESULT_SCHEMA_REJECTED`. The stable public code inventory is frozen in `conformance/public-api.json`; raw resolver, handler, and internal exception text is never returned.

R0 execution uses the normal dispatcher without authorization material. For R1-R4, the embedding application may provide an `AuthorizationResolver`. The resolver receives the immutable exact dispatch request and inspected target, and must return the policy snapshot and already-consumed authorization receipt for that request. The dispatcher compares every receipt field with the durable grant and atomically redeems the exact receipt before invoking the handler. Policy denials map to `AUTHORIZATION_DENIED`; only failures in the resolver itself map to `AUTHORIZATION_RESOLVER_FAILED`. A missing, rejected, malformed, replayed, or failing authorization path stops before handler invocation.
