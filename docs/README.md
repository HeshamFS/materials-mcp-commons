# Public documentation

This directory contains public, product-facing documentation for Materials MCP Commons.

- [Architecture](architecture.md)
- [Engine runtime](runtime.md)
- [Scientist Kit authoring](authoring.md)
- [Materials MCP Profile contracts](contracts.md)
- [Canonical schema hosting](schema-hosting.md)
- [MCP host adapter](mcp-host.md)
- [Engine operations runbook](operations-runbook.md)
- [Durable state recovery](state-recovery.md)
- [Engine host threat model](threat-model.md)
- [Development foundation](development.md)
- [Scientific evidence policy](scientific-evidence.md)
- [Versioning and compatibility](versioning.md)
- [Licensing and third-party material](licensing.md)

Documentation grows alongside executable contracts and verified capabilities. It describes public product behavior and supported evidence without treating schema validity as runtime or scientific validation.

Unless a file states otherwise, documentation and specification prose are licensed under CC BY 4.0. See [`../LICENSE-DOCS.md`](../LICENSE-DOCS.md).
