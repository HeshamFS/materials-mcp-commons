...
| Effectful work bypasses approval or quota | R1-R4 require the policy engine plus exact consumed receipt; absent resolver fails before handler invocation | The host application must implement the real approval UX and protect its durable policy state |
