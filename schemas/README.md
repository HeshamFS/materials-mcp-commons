...
Each contract version has one exact-version directory, including immutable [`0.1.0/`](0.1.0/README.md) and its additive [`0.2.0/`](0.2.0/README.md) successor. Mutable aliases such as `latest` are not permitted. Every root schema declares the Draft 2020-12 dialect and a path-derived, absolute, fragment-free identifier under:

`https://schemas.autonomouslab.io/materials-mcp/{profile-version}/{resource-name}.schema.json`

The 0.1.0 line contains 16 normative root schemas and a [checksum-pinned resource index](0.1.0/schema-index.json). Profile 0.2.0 carries fifteen scientific/control contracts forward, evolves the version-specific schema index, and adds compact-result, context-manifest, and profile-compatibility schemas. Validation is deliberately offline: implementations pre-register the exact resources they support and fail closed when a reference or extension schema is absent. Dereferenceable HTTPS copies are a distribution channel, not a validation-time trust source.
